...
//         'relation'        => 'AND',
//         array(
//             'key'        => 'sme_additional_info_languages',
//             'value'        => 'English',
//             'compare'    => 'LIKE'
//         )
//     )
...
// sme, SME - post, name: 'sme_additional_info', 
...
function BuildPostTypeFilters($post_type)
{
    ...
    $fields = acf_get_fields($groups[array_search('SME - post', array_column($groups, 'title'))]['key']);
    $fields = $fields[array_search('sme_additional_info', array_column($fields, 'name'))]['sub_fields'];

    // Now loop through the groups to target the lowest level
    // $fields = null;
    // foreach ($group_list as $group) {
    //     $fields = acf_get_fields($groups[array_search($group_name, array_column($group, 'title'))]['key']);
    // }
    ...
    foreach ($fields as $field) {
        if (in_array($field['type'], ['checkbox', 'select']))
            array_push($items, (object) [
                "label" => $field['label'],
                "name" => 'sme_additional_info_' . $field['name'],
...
// $assetItems = BuildPostTypeFilters('sme', [['title' => 'SME - post'], 'sme_additional_info'], 'sub_fields');


$my_posts = BuildSearchQueryFromPost('sme');

function trim_tag_count($tags_to_trim, $char_limit)
{
    $extra_message = "+%d more";
    $char_limit = $char_limit - strlen($extra_message);

    $current_count = 0;
    $new_tag_list = [];
    foreach ($tags_to_trim as $k => $v) {
        if ($current_count + strlen($v) < $char_limit) {
            array_push($new_tag_list, $v);
            $current_count += strlen($v);
        } else {
            array_push($new_tag_list, sprintf($extra_message, count($tags_to_trim) - $k));
            break;
        }
    }
    ...
    <div class="bx--tile-container" style="width: 100%">
        ...
        <div class="bx--row">
            <div class="bx--col-lg-3 sp__filter-column">
                <form id="filterform" method="GET" action="<?php echo esc_url(home_url('/search-sme/')); ?>">
                    <div class="sp__search_bar">
                        <div class="bx--form-item">
                            <input type="text" class="bx--text-input" placeholder="Search Experts" name="__text_search" value="<?php echo $_GET['__text_search']; ?>">
                        </div>
                    ...
                    <br>
                    <?php
                    $assetItems = BuildPostTypeFilters('sme');
                ...
                ?>
                        <div class="bx--tile--clickable sp__asset_tile" onclick="location.href='<?php echo get_permalink(); ?>'">
                            ...
                            <div class="sp__tile_body">
                                <div class="sp__tile_sme_header">
                                    <?php the_title(); ?>
                                </div>
                                <div class="sp__tile_job_title">
                                    <?php echo get_field('sme_speaker_info_job_title'); ?>
                                </div>
                                <div class="sp__tile_sme_gap"></div>
                                <div class="sp__sme_tags">
                                    <?php
                                    $tags = get_field('sme_additional_info_focus');
                                    $tags = trim_tag_count($tags, 60);
                                    foreach ($tags as $tag) {
                                    ?>
                                        <button class="bx--tag bx--tag--gray sp__sme_tag">
                                            <span class="bx--tag__label"><?php echo $tag ?></span>
                                        </button>
                                    <?php
                                    }
                                    ?>
                                </div>
                                <div class="sp__asset_fake_link">View</div>
                            </div>
                        </div>
                    <?php
                    }
